...
/**
 * MASAR - Applications Payment Confirm Case Runner (internal)
 *
 * One-shot subprocess executed by tests/applications_payment_manual_regression.php.
 * NOT a standalone test.
 *
 * Usage: php tests/applications_payment_confirm_case.php <guest|bearer> <token> [request_uri]
 *
 * It boots the same request shim as public/index.php (minus static asset /
 * HTML handling), issues the given POST request (defaults to
 * /api/v1/applications/{id}/payment/confirm), and prints:
 *
 *     STATUS=<http_code>
 *     BODY=<raw json body>
 */
...
$token    = (string) ($argv[2] ?? '');
$request_uri = (string) ($argv[3] ?? '/api/v1/applications/payment/confirm');
