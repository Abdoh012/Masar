 ...
 *            specialization(s). A logged-in student with Specialization A must
 *            never see a published training that belongs to Specialization B,
 *            even if the search text matches that training's fields.
 *
 ...
 *   3. Multi-specialization scope (array of ids) OR-matches all assigned specs.
...
try {
    ...
    $insTraining = $pdo->prepare(
        "INSERT INTO training_listings
            (id, company_id, title, description, training_type, mode, is_paid, status,
    ...
    $insTraining->execute([$trainingAid, $companyCid, "Spec A {$kw} Engineering", "Spec A only {$kw}"]);
    $createdTrainings[] = $trainingAid;
    $insTraining->execute([$trainingBid, $companyCid, "Spec B {$kw} Marketing", "Spec B only {$kw}"]);
    ...
    echo "\n== TEST3: multi-specialization scope (array of ids) OR-matches ==\n";
