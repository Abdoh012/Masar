...
function company_validator_create(
    ...
    */
    ...
    | study field IDs. The industry holds specialization reference(s): a
    | single specialization ID (int or numeric string) or name, or an array
    | of IDs and/or names, resolved against the specializations lookup table
    | in the service layer. At least one work field or one industry is
    | required.
    |
    */
    ...
    if (is_int($industry_input) || (is_string($industry_input) && ctype_digit($industry_input))) {

        if ((int) $industry_input > 0) {

            $has_industries = true;
        }
    } elseif (is_string($industry_input)) {
        ...
        foreach ($industry_input as $industry_value) {

            if (
                is_int($industry_value)
                ||
                (
                    is_string($industry_value)
                    &&
                    ctype_digit($industry_value)
                )
            ) {
            ...
            if (
                strlen(trim($industry_value)) > 255
            ) {
