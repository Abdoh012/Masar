...
|--------------------------------------------------------------------------
|
| Returns true when the specialization id references an active row in the
| specializations lookup table. Used to validate the required
| specialization_id sent by a company when creating a training.
|
*/

function training_repository_specialization_exists(
...
function training_repository_build_public_query(
    ...
    /*
    |--------------------------------------------------------------------------
    | Specialization-Based Student Matching
    |--------------------------------------------------------------------------
    |
    | Restricts results to trainings whose single primary specialization
    | (training_listings.specialization_id) is one of the given
    ...
    */
    ...
    if (
        !empty($filters['match_specialization_ids'])
        &&
        is_array($filters['match_specialization_ids'])
    ) {
...
function training_repository_get_skills_by_training_ids(
    ...
    foreach ($rows as $row) {
        ...
        $grouped[$training_id][] = $row['skill_name'];
    }
...
|--------------------------------------------------------------------------
|
| Returns the single specialization of a training as an {id, name} object,
| or null when the training has no specialization. Powers the `specialization`
| key in training responses (one-specialization-per-training contract).
|
*/

function training_repository_get_specialization(
    int $specialization_id
): ?array {
    ...
    return [
        ...
        'name' => (string) $row['specialization_name'],
    ];
}
