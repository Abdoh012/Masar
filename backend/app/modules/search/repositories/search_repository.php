...
function search_repository_attach_training_relations(array $items, bool $is_student = false, ?int $student_specialization_id = null): array {
    ...
    foreach (
        ...
        $skills[$row['training_id']][] = $row['name'];
    }
...
function search_repository_student_specialization_ids(int $student_id): array {
    /*
     * Returns the specialization id assigned to a student. In the
     ...
     * most one id. The array shape keeps the IN-matching scope reusable
...
function search_repository_specialization_scope(array $specialization_ids): string {
    /*
     * Builds a WHERE fragment that restricts the current training row
     * (aliased t) to those whose single primary specialization
     * (training_listings.specialization_id) is one of the given ids.
     * The student holds one specialization_id (there is no
     * student_specializations pivot), so this is an exact column match;
     * an IN-list keeps the fragment correct if the input ever grows.
     * Every id is cast to int before being inlined, so no user input
    ...
    return " AND t.specialization_id IN ("
...
function search_repository_trainings(string $query, array $filters = []): array {
     ...
     * skills and the training's single specialization through their
    ...
    $is_student = $student_id > 0;
    $student_specialization_id = $is_student
        ? ((search_repository_student_specialization_ids($student_id))[0] ?? null)
        : null;
    ...
    $where = "(t.title LIKE :kw_title
        ...
        OR EXISTS (
            SELECT 1 FROM specializations kws2
            WHERE kws2.id = t.specialization_id
    ...
    return ['items' => search_repository_attach_training_relations($items, $is_student, $student_specialization_id), 'total' => $total];
}
