...
/*
|--------------------------------------------------------------------------
| Set Training Primary Specialization
|--------------------------------------------------------------------------
|
| Assigns the training's single primary specialization
| (training_listings.specialization_id, NOT NULL) from the company's
| industry. The first of the company's specializations is used
| deterministically, mirroring the migration backfill (MIN id).
|
*/

function authtest_set_training_primary_specialization(
    PDO $pdo,
    int $company_id,
    int $training_id
): void {
    ...
    $pdo->prepare("
        ...
        SET t.specialization_id = (
            SELECT MIN(cs.specialization_id)
...
function authtest_find_or_create_training(
    ...
    if ($existing !== false) {
        $training_id = (int) $existing;
        authtest_set_training_primary_specialization(
            $pdo,
            $company_id,
            $training_id
        );
